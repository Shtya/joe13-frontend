  ...
  useEffect(() => {
    if (typedKey) {
      const index = data.findIndex((e) =>
        (e?.name_en || e?.name)
          ?.toLowerCase()
          .startsWith(typedKey.toLowerCase())
      );
    ...
    >
      ...
      >
        <div className=" flex items-center px-[10px] gap-[9px] P-12  ">
          <div
            className={`text16 ${
              val?.name ? "text-white " : "text-[#9ca3af]"
            } `}
          >
            {" "}
            {val?.name || place}{" "}
      ...
      >
        {data.map((e, i) => (
          <div
            ...
            className=" hover:bg-gray1 hover:bg-primary  P-12 duration-100 min-h-[35px] cursor-pointer flex items-center px-[10px] "
            key={i}
            onClick={(_) => handleValue(e)}
          >
            <span className="capitalize text16 hover:bg-primary duration-0 ">
              {" "}
              {e?.name_en ? e[`name_${locale}`] : e?.name}{" "}
            </span>
          </div>
        ))}
      </div>
