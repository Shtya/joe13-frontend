...
const Footer = ({ cn, id }) => {
  ...
  return loading ? (
        ...
        {[...Array(6)].map((_, i) => (
          <div key={i} className="col  space-y-4">
            ...
            {[...Array(3)].map((_, j) => (
              <div
                key={j}
                className="h-4 w-full skeleton-box bg-white/10 rounded"
              />
            ))}
          </div>
        ))}
    ...
    <footer
      id={id}
      className={`  bg-white  z-[1000] text-black max-md:pt-[50px] py-[50px] ${cn} `}
    >
      <div className=" lg:pr-[70px] px-[20px] max-w-[1500px] w-full mx-auto  grid grid-cols-5 gap-[30px] max-xl:grid-cols-3 max-lg:grid-cols-2 max-sm:grid-cols-1 ">
        <div className="col ">
          {settings?.site_logo && (
            <div className="relative mt-[-15px] max-w-[200px] w-full h-[50px]">
              <Image
                className="object-contain object-left"
                src={baseImage(settings?.site_logo?.url)}
                alt={settings?.site_logo?.alt}
                fill
                sizes="200px"
                priority
              />
          ...
          <p className="text-black/90 text-sm max-w-[400px] w-full  mb-[10px] mt-[-5px]   ">
            {" "}
            {settings?.about_us_footer?.[locale]}{" "}
          </p>
          <div className="flex items-center flex-wrap justify-center gap-[5px] max-md:gap-[5px] max-w-fit w-full ">
            {Object.entries(settings?.social_media || {}).map(
              ([key, value], i) => (
                <a
                  key={i}
                  aria-label={key}
                  className=" flex items-center justify-center  btn-blue-3d w-[35px] h-[35px] max-md:w-[30px] max-md:h-[30px]  rounded-full hover:scale-[1.1]  p-[8px]  duration-300 "
                  href={value}
                >
                  <Image
                    className="object-contain"
                    src={`/assets/social/${key}.png`}
                    alt={key}
                    width={16}
                    height={16}
                  />
        ...
        <div className="col">
          ...
          <div className="flex items-center gap-[5px] mb-[2px]">
            ...
            <div
              className={`${style.link} !mb-0 hover:!text-black !cursor-context-menu `}
            >
              {" "}
              {settings?.contact_us?.address?.[locale]}{" "}
            </div>
          </div>
          ...
          <div className="flex items-center gap-[5px] mb-[2px]">
            ...
            <a
              href={`mailto:${settings?.contact_us?.email}`}
              className={`${style.link} !mb-0 cursor-pointer underline hover:!text-black `}
            >
              {" "}
          ...
          <div className="flex items-center gap-[5px] mb-[2px]">
            ...
            <a
              href={`https://wa.me/${settings?.contact_us?.phone}`}
              target="_blank"
              rel="noopener noreferrer"
              style={{ direction: "ltr" }}
              className={`${style.link} !mb-0 cursor-pointer underline hover:!text-black  rtl:text-right `}
            >
              {" "}
        ...
        <div className="col">
          ...
          {settings?.branch?.[locale]?.map((e, i) => (
            <div key={i} className="flex items-center gap-[5px] mb-[4px] ">
      ...
      <p className="mt-[20px] text-sm  mb-[-30px] text-black/80 text-center  ">
        {" "}
